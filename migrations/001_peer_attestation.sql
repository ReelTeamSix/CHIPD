-- ============================================================================
-- CHIP'D Migration: v1.0.0 → v1.1.0
-- Peer Attestation with Foursome Support
-- ============================================================================
-- 
-- Run this in Supabase SQL Editor to migrate from v1.0.0
-- 
-- WHAT THIS DOES:
-- 1. Simplifies nfc_tags to player-owned only (removes course-mounting fields)
-- 2. Adds round_players table for 2-4 player support
-- 3. Updates rounds table for GPS verification
-- 4. Migrates existing round data
--
-- ============================================================================


-- ============================================================================
-- STEP 1: Update NFC_TAGS table (remove course-mounting, add nickname)
-- ============================================================================

-- Remove course-mounted tag fields (we use GPS instead)
...
-- Add nickname for friendly tag names
ALTER TABLE public.nfc_tags ADD COLUMN IF NOT EXISTS nickname TEXT;

-- Make owner_id required (all tags are player-owned)
-- First, delete any orphan tags without owners
...
-- ============================================================================
-- STEP 2: Create ROUND_PLAYERS table (supports 2-4 players, verified or not)
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS public.round_players (
    ...
    -- Attestation: has this player confirmed the round?
...
-- Players can see rounds they're part of
...
CREATE POLICY "Round creator can add players"
    ...
    WITH CHECK (
        ...
        OR user_id = auth.uid()  -- Players can add themselves
    );

-- Players can update their own participation (scores, attestation)
...
COMMENT ON TABLE public.round_players IS 'Players in a round (2-4 per round). Each player has their own scores and attestation status.';
...
-- Add group size tracking (allow 1 for solo practice)
ALTER TABLE public.rounds ADD COLUMN IF NOT EXISTS player_count INTEGER DEFAULT 2;

-- Add verification level tracking (full/partial/none)
...
-- Add 'pending' to status if not already there
...
ALTER TABLE public.rounds ADD CONSTRAINT rounds_status_check 
    CHECK (status IN ('pending', 'in_progress', 'completed', 'verified', 'disputed'));

-- Remove old single-player fields (after migrating data)
-- Keep user_id for now as created_by reference, remove later if needed

-- Remove old signature fields (we use GPS + NFC scan timestamps now)
ALTER TABLE public.rounds DROP COLUMN IF EXISTS start_tap_signature;
ALTER TABLE public.rounds DROP COLUMN IF EXISTS end_tap_signature;
...
DROP POLICY IF EXISTS "Users can update own in-progress rounds" ON public.rounds;
...
-- ============================================================================
-- STEP 4: Migrate existing round data to round_players
-- ============================================================================

-- If there's existing data with user_id, create round_players entries
INSERT INTO public.round_players (round_id, user_id, scores, total_score, has_attested)
SELECT 
    id,
    user_id,
    scores,
...
AND NOT EXISTS (
    SELECT 1 FROM public.round_players rp WHERE rp.round_id = rounds.id AND rp.user_id = rounds.user_id
);
...
-- ============================================================================
-- STEP 10: Create indexes for games
-- ============================================================================
...
-- ============================================================================
-- STEP 11: Add tier to users table
-- ============================================================================
...
-- ============================================================================
-- STEP 12: Create SUBSCRIPTIONS table
-- ============================================================================
...
-- ============================================================================
-- STEP 13: Create tier sync trigger
-- ============================================================================
...
VALUES ('1.1.0', 'Peer Attestation: round_players, dynamic games, chip transactions, subscriptions, tier system')
